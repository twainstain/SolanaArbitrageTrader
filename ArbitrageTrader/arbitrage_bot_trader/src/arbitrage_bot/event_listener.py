...
Usage::
    ...
    PYTHONPATH=src python -m arbitrage_bot.event_listener \\
        --config config/uniswap_pancake_config.json --dry-run

Architecture:
  - Polls the latest block for Swap events on configured pool(s)
  - When a swap is detected, fetches fresh quotes and evaluates
...
from arbitrage_bot.env import get_rpc_overrides, load_env
...
class SwapEventListener:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            logger.info(
                "Stopped. Swaps detected: %d, Opportunities: %d",
                self._swap_count, self._opportunity_count,
            )
            log_summary(
                logger, "EVENT-LISTENER", self._scan_index,
                self._opportunity_count, 0, 0.0, self.config.base_asset,
            )
    ...
    def _poll_once(self, pool_addrs: list[str]) -> None:
        ...
        # Re-check profitability after the swap(s).
        try:
        ...
        opportunity = self.strategy.find_best_opportunity(quotes)
        ...
        logger.info(
            "OPPORTUNITY: buy on %s, sell on %s, net=%.6f %s",
        ...
        if self.dry_run:
            ...
            if result.success:
                logger.info(
                    ...
                    result.realized_profit_base, self.config.base_asset,
                )
